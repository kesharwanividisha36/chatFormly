...
function Sidebar() {
 ...
 fixed bottom-[20px] left-[20px] bg-[#39fc12]'onClick={handleLogOut} >
...
{searchData?.map((user)=>(
     ...
     <h1 className='text-gray-800 font-semibold text-[20px]'>{user.name || user.userName}</h1>
      ...
      <div className='w-full h-[300px] bg-[#3aff20] rounded-b-[30%] shadow-gray-400 shadow-lg flex flex-col justify-center px-[20px] '>
    <h1 className='text-white font-bold text-[25px]'>chatFormly</h1>
...
{!search && otherUsers?.map((user)=>(
    ...
    <span className='w-[12px] h-[12px] rounded-full absolute bottom-[6px] right-[-1px] bg-[#1c64eb] shadow-gray-500 shadow-md'></span>
    </div>
))}
...
{otherUsers?.map((user)=>(
    <div className='w-[95%] h-[60px] flex items-center gap-[20px] shadow-gray-500 bg-white shadow-lg rounded-full hover:bg-[#a4f665] cursor-pointer' onClick={()=>dispatch(setSelectedUser(user))}>
...
export default Sidebar
        {/* <div className='w-full h-[300px] bg-[#39fc12] rounded-b-[30%] 
      shadow-gray-400 shadow-lg  justify-center flex flex-col
       px-[20px]'>
        
<h1 className='text-white font-bold text-[25px]'> ChatFomrly</h1>
<div className='w-full flex justify-between items-center'>
    <h1 className='font-bold text-[25px] text-gray-700'>Hii, {userData.name || "user"}</h1>
    <div className='w-[60px] h-[60px] rounded-full overflow-hidden flex justify-center items-center
    shadow-lg shadow-gray-500 cursor-pointer bg-white' onClick={()=>navigate("/profile")}>
                <img src={userData.image || dp} alt="" className='h-[100%]'/> 
</div>
    </div>
    <div  className='w-full flex items-center gap-[20px]'>
        {!search &&  <div className='w-[60px] h-[60px] rounded-full overflow-hidden flex justify-center items-center
    shadow-lg shadow-gray-500 bg-white' onClick={()=>setSearch(true)}>
    <IoSearch className='w-[25px] h-[25px]' />
</div>}
{search &&
 <form className='w-full h-[60px] bg-white shadow-lg shadow-gray-500
flex items-center gap-[10px] mt-[10px] rounded-full overflow-hidden px-[20px]'>
    <IoSearch className='w-[25px] h-[25px] cursor-pointer'/>
    <input type="text" placeholder='search users..' className='w-full h-full
    p-[10px] text-[17px] outline-0 border-0'/>
    <RxCross2 className='w-[25px] h-[25px] cursor-pointer' onClick={()=>setSearch(false)} />
    </form>
    }
{!search && otherUsers?.map((user)=>(
    <div className='w-[60px] h-[60px] rounded-full overflow-hidden flex justify-center items-center
    shadow-lg shadow-gray-500 bg-white'>
                <img src={user.image || dp} alt="" className='h-[100%]'/> 
</div>
))}
      ...
      <div className='w-full h-[60vh] overflow-auto flex flex-col gap-[20px] items-center
      mt-[20px] '>
        {otherUsers?.map((user)=>(
        <div className='w-[95%] h-[60px] flex justify-start items-center
        gap-[20px] bg-white shadow-lg shadow-gray-500 rounded-full hover:bg-[#b3ea4eb0] cursor-pointer'
