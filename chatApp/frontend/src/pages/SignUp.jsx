...
setLoading(false)
  ...
  return (
    ...
    <div className='w-full h-[100vh] bg-slate-200 flex items-center justify-center'>
     ...
     shadow-gray-400 shadow-lg flex flex-col gap-[30px]'>
      ...
      shadow-gray-400 shadow-lg items-center justify-center flex'>
        <h1 className='text-gray-700 font-bold text-[30px]'>Welcome to <span className='text-white'>ChatFomrly</span></h1>

      </div>
      <form className='w-full flex flex-col gap-[20px] items-center justify-center'onSubmit={handleSignUp}>
    <input type="text" placeholder='username' className='w-[90%] h-[50px]
    outline-none border-2 border-[#39fc12] px-[20px] py-[10px] bg-[white]
    rounded-lg shadow-gray-200 shadow-lg text-[19px] text-gray-700' 
     onChange={(e)=>setUserName(e.target.value)} value={userName}/>
    <input type="text" placeholder='email' className='w-[90%] h-[50px]
    outline-none border-2 border-[#39fc12] px-[20px] py-[10px] bg-[white]
    rounded-lg shadow-gray-200 shadow-lg text-[19px] text-gray-700'
    ...
    <p className='cursor-pointer' onClick={()=>navigate("/login")}>Already Have An Account ?<span className='text-[#39fc12] font-bold'>Login</span></p>
     </form>
     </div>
